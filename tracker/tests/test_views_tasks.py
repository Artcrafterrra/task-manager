...
class TestTaskViews(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="u1", password="pass12345", position=self.pos
        )
        self.other = User.objects.create_user(
            username="u2", password="pass12345", position=self.pos
        )
    ...
    def test_task_list_filters(self):
        ...
        self.assertCountEqual(
            resp.context["object_list"], [self.task1, self.task2, self.task3]
        )

        resp = self.client.get(base, {"my": "1"})
        self.assertCountEqual(
            resp.context["object_list"], [self.task1, self.task3]
        )

        resp = self.client.get(base, {"created": "1"})
        self.assertCountEqual(
            resp.context["object_list"], [self.task1, self.task2]
        )
    ...
    def test_task_toggle_complete(self):
        ...
        resp = self.client.post(
            url, HTTP_REFERER=reverse("tracker:task-list")
        )
        self.assertEqual(resp.status_code, 302)
        self.task1.refresh_from_db()
        self.assertTrue(self.task1.is_completed)
